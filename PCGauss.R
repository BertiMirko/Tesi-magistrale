#' Structure learning with Gaussian models
#'
#' This function estimates the adjacency matrix of a Gaussian model given a
...
gauss.wald <- function(X,maxcard,alpha,extend){
  p <- ncol(X)
  n <- nrow(X)
  adj <- matrix(1,p,p)
  diag(adj) <- 0
